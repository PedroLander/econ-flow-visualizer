...
/// Process FIGARO trade flow data and return a list of (source, target, value) tuples
#[pyfunction]
...
fn process_tsv(file_path: &Path) -> polars::error::PolarsResult<DataFrame> {
...
fn get_flow_data(imports_path: &Path, exports_path: &Path, year: i32) -> polars::error::PolarsResult<Vec<(String, String, f64)>> {
    ...
    let process_df = |df: &DataFrame, flow_type: &str| -> polars::error::PolarsResult<Vec<(String, String, f64)>> {
